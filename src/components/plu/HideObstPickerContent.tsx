// Inhalt der Vollseite / ehemaliger Dialog: Obst – Produkte ausblenden (Suche, Mehrfachauswahl, Batch)
...
export interface HideObstSearchableItem {
  id: string
  plu: string
  display_name: string
  system_name?: string
...
type SearchableItem = HideObstSearchableItem

const DEFAULT_FONT_SIZES: DialogPluFontSizes = { header: 24, column: 16, product: 12 }

export interface HideObstPickerContentProps {
...
export function HideObstPickerContent({
  searchableItems,
  displayMode = 'MIXED',
  listLayout,
  ...
  return (
    ...
    >
      <div>
        <h2 className="text-xl font-semibold tracking-tight">Produkte ausblenden</h2>
        ...
        {filteredItems.length === 0 ? (
          <div className="flex-1 flex items-center justify-center p-8">
            <p className="text-sm text-muted-foreground text-center">
              {searchText.trim()
                ? 'Keine Treffer.'
                : 'Keine Produkte zum Ausblenden. Alle sind bereits ausgeblendet.'}
            </p>
          </div>
        ) : (
              ...
              <div className="hidden md:block">
                <table className="w-full table-fixed">
                  ...
                  <thead className="sticky top-0 bg-background z-10">
                    <tr className="border-b-2 border-border">
                      <th className="px-1 py-1.5 w-[36px]">
                        <Label htmlFor="select-all-np" className="sr-only">
                          Alle auswählen
                        </Label>
                        <Checkbox
                          id="select-all-np"
