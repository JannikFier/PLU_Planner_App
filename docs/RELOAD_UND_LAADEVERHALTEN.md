...
1. **Auth:** Wenn beim Reload ein gültiges Profil + Session im `sessionStorage` liegen (`PROFILE_CACHE_KEY`, `SESSION_CACHE_KEY`), setzt der AuthContext sofort `isLoading: false` und zeigt die App mit gecachtem User/Profil. Der echte Session-Check (`getSession`) läuft danach (120ms oder `requestIdleCallback`). **Ohne** Cache: erst `getSession()`, dann `fetchProfile()` → spürbare Verzögerung und Fullscreen-Spinner. Das **Supabase-JWT** liegt lokal in **`sessionStorage`**; in **Produktion** (wenn `VITE_APP_DOMAIN` nicht `localhost`) in **Cookies** mit `Domain=.<Basisdomain>` ([`supabase-auth-cookie-storage.ts`](../src/lib/supabase-auth-cookie-storage.ts)). Reload **im selben Tab** behält die Anmeldung; **Tab schließen** bei localhost beendet sie lokal; in Produktion Cookies bis Abmelden / Ablauf.
...
**Dateien:** [src/App.tsx](../src/App.tsx) (Persister, `PersistQueryClientProvider`, dehydrateOptions), [src/lib/query-client.ts](../src/lib/query-client.ts) (gemeinsamer `QueryClient`), [src/lib/query-persist-allowlist.ts](../src/lib/query-persist-allowlist.ts) (Allowlist).
...
- **Zentrale Prefetch-Funktionen** bleiben in [src/hooks/usePrefetchForNavigation.ts](../src/hooks/usePrefetchForNavigation.ts). Dort sind u. a. `runMasterListPrefetch`, `runAdminPrefetch` und `runSuperAdminCompaniesPrefetch` (Firmenliste für Super-Admin) definiert.
- **Regel dokumentieren:**  
