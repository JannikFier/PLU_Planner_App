...
Pro Markt existiert ein **Einstiegs-Link** (langer Token in der URL). Nach dem Scannen wählt die Kasse eine **Kasse** (Slot) und gibt das **Passwort** ein; die Edge Function `kiosk-login` liefert eine Session. Die technische Profil-Rolle **`kiosk`** entspricht datenbankseitig Leserechten wie der Viewer, in der App aber **ohne PDF-Export** und nur unter **`/kiosk/*`** (kein Marktwechsler, kein Admin-Bereich). **Admin/Super-Admin** pflegen QR, Kassen und Rotation unter **Administration → Kassenmodus** (`/admin/kassenmodus`); in der **Benutzerverwaltung** verweist eine Kachel „Kassen & QR“ auf dieselbe Seite. Zusätzliche Ideen (Meldungen an der Kasse, Subdomain-only) siehe [FEATURE_KASSEN_SPEC.md](FEATURE_KASSEN_SPEC.md).
