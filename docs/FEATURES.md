...
| Phase 7 | Deployment (Vercel, Domains, Kassen-QR) | Anleitung: [DEPLOYMENT_DOMAINEN_UND_KASSE.md](DEPLOYMENT_DOMAINEN_UND_KASSE.md) |
...
## Refactor-Roadmap (Stufe 4 / 5)

Große Seiten schrittweise verdünnen (Hooks/Lib), Virtualisierung der PLU-Hauptlisten erst nach Messung und **M0 Go**: Überblick [REFACTOR_ROADMAP_STUFEN.md](REFACTOR_ROADMAP_STUFEN.md), Detail-Stufe 4 [REFACTOR_STUFE_4_AGENT_PLAN.md](REFACTOR_STUFE_4_AGENT_PLAN.md), Virtualisierung [REFACTOR_STUFE_5_AGENT_PLAN.md](REFACTOR_STUFE_5_AGENT_PLAN.md) und [STUFE_5_M0_CHECKLIST.md](STUFE_5_M0_CHECKLIST.md).
...
**Markt-Login-Link (Super-Admin):** In den **Markt-Einstellungen** eines Marktes gibt es **„Kopieren“** neben der Subdomain – kopiert **`https://{subdomain}.{VITE_APP_DOMAIN}/login`** ([`buildMarketLoginUrl`](src/lib/subdomain.ts)) für das Personal.

**QR- und Einstiegs-URL (Markt-Subdomain):** Wenn der Markt eine gültige **`stores.subdomain`** hat und `VITE_APP_DOMAIN` gesetzt ist, bauen QR und kopierter Link **`https://{subdomain}.{VITE_APP_DOMAIN}/kasse/{token}`** ([`buildKioskEntranceUrl`](src/lib/kiosk-entrance-url.ts)) – erkennbarer Markt-Host. **Lokal (Vite):** Der Port der aktuellen Seite (z. B. `:5173`) wird an `http://{subdomain}.localhost…` angehängt, damit der neue Tab denselben Dev-Server trifft. Ohne nutzbare Subdomain: Fallback auf die **aktuelle Origin** (z. B. reines Vercel-Preview). **DNS:** Wildcard `*.deine-domain.de` auf das Hosting legen, damit Markt-Hosts erreichbar sind. **Schritt-für-Schritt (Vercel, Wildcard, Supabase, Rollout):** [DEPLOYMENT_DOMAINEN_UND_KASSE.md](DEPLOYMENT_DOMAINEN_UND_KASSE.md). In der Verwaltung **Kassenmodus** erscheint bei offensichtlich falscher Production-Konfiguration (Link noch `*.localhost`) eine rote Hinweisbox.

**Supabase:** `VITE_SUPABASE_URL` / `VITE_SUPABASE_ANON_KEY` wie gewohnt.

**Session und Tabs (Auth-JWT):** **Lokal (`localhost`):** Supabase-Auth liegt in **`sessionStorage`** – Tab schließen beendet die Anmeldung lokal; neuer Tab zur gleichen Origin hat eigenes Storage. **Produktion** (wenn `VITE_APP_DOMAIN` nicht `localhost` ist): Die Session wird in **HTTP-Cookies** mit **`Domain=.<Basisdomain>`** gehalten ([`supabase-auth-cookie-storage.ts`](src/lib/supabase-auth-cookie-storage.ts)), damit **`www`** und **`{markt}.Basisdomain`** dieselbe Anmeldung teilen (Marktwechsel im Header, Redirect nach Login). Cookies haben ein langes Max-Age (nicht nur „bis Tab zu“); für getrennte PCs weiterhin **Abmelden** nutzen. **Kasse vs. Verwaltung:** Unterschiedliche Origins (z. B. `app.` vs. Markt-Subdomain) haben weiterhin **getrenntes** Cookie-Storage. **Neuer Tab** auf derselben Parent-Domain teilt die Session mit den Cookies. TanStack-Query-Persistenz bleibt in `sessionStorage` (pro Subdomain isoliert).
...
In der **Benutzerverwaltung** sieht der Super-Admin nur Nutzer, die mindestens einem Markt der **im Kopfbereich gewählten Firma** zugeordnet sind. Ohne gewählte Firma/Markt erscheint ein Hinweis statt der Liste. **Admins** sind weiterhin über die Datenbankregeln (RLS) auf die eigene Firma beschränkt. **Kassen-Konten** (`kiosk`) stehen nicht in der Tabelle „Alle Benutzer“, sondern unter dem Abschnitt **Kassen & QR** für den gewählten Markt (wie im Kassenmodus: Passwort setzen, aktivieren/deaktivieren, löschen); Anlegen und QR weiterhin unter **Kassenmodus**. Unter **Super-Admin → Firma → Markt → Benutzer** gilt dasselbe: nur Personal in der Benutzer-Tabelle, Kassen im Abschnitt **Kassen & QR** darunter.
...
- **Master Products:** Admin und Super-Admin können umbenennen.
  ...
  - **Vollseite „Produkte umbenennen“ (Obst):** Einfache Suchleiste (PLU/Name) wie beim Ausblenden-Picker – filtert die Liste, Treffer werden hervorgehoben, optional Scroll zum ersten Treffer. Kein „X von Y“, keine Pfeil-Buttons. Pro Zeile ein Stift; Klick öffnet weiterhin den **Modal** „Produkt umbenennen“. Nach Speichern bleibt die Picker-Seite offen, die Listen werden aktualisiert. **Desktop-Layout** entspricht den **Layout-Einstellungen**: Obst/Gemüse bei **spaltenweise** wie die Masterliste (Zeitung inkl. Seiten-Trennern bei Bedarf), bei **zeilenweise** zwei Artikel pro Zeile (PLU \| Artikel \| PLU \| Artikel). **Backshop:** gleiche Picker-Logik unter **`*/pick-rename-backshop`** (inkl. Bild und E/H/A-Badge wie beim Backshop-Ausblenden-Picker). Zusätzlich **„Meine Liste“** / **„Alle Produkte“** (Auswahl wird im Browser gespeichert; **Suchfeld links**, Umschalter **rechts**): *Meine Liste* = dieselben **Master-/Carryover-Zeilen** wie in der sichtbaren Backshop-PLU-Tabelle (Zuordnung über dieselbe Zeilen-`id` wie die Layout-Engine, damit bei gleicher PLU für mehrere Quellen die gewählte Marke stimmt); *Alle Produkte* = vollständiger Katalog der Version inkl. Carryover wie bisher (weiterhin höchstens eine Zeile pro PLU im Picker).
